...
public class PetRepositoryIntegrationTest {
    ...
    private final Logger log = LoggerFactory.getLogger(PetRepositoryIntegrationTest.class);
...
//        pet.setId(1L);
    ...
    public void testFindOne() {
        Pet petPersisted = petRepository.findOneByName("Garfield");
        assertNotNull(petPersisted);
        assertEquals("Garfield", petPersisted.getName());
        assertEquals(pet.getAge(), petPersisted.getAge());
        assertEquals(pet.getNotes(), petPersisted.getNotes());
        assertEquals(Gender.MALE, petPersisted.getGender());
        assertEquals(pet.getBreed(), petPersisted.getBreed());
        assertEquals(3.5F, petPersisted.getWeight(), 0);
        assertEquals(pet.getType(), petPersisted.getType());
    }
